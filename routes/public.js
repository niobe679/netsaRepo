...
router.post('/main/search', async (req, res) => {
  ...
  try {
    ...
    const filters = {};
    const { minPrice, maxPrice, bedrooms, bathrooms, squarefeet, location, type, rob } = req.body;
    ...
    if (rob) {
      const robInt = parseInt(rob);
      if (!isNaN(robInt)) {
        filters.rob = robInt;
      } else {
        filters.rob = { $regex: rob, $options: 'i' };
      }
    }

    if (location) filters.location = { $regex: location, $options: 'i' };
    if (bedrooms && bedrooms !== "Any") filters.bedrooms = { $gte: parseInt(bedrooms) };
    if (bathrooms && bathrooms !== "Any") filters.bathrooms = { $gte: parseFloat(bathrooms) };
    if (squarefeet) filters.squarefeet = { $gte: parseInt(squarefeet) };
